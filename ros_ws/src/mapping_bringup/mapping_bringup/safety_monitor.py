...
class SafetyMonitor(Node):
    ...
    def _read_throttled_value(self, path):
        try:
            if not os.path.exists(path):
                return None
            return int(open(path, 'r', encoding='utf-8').read().strip(), 0)
        except Exception:
            return None

    def _read_vcgencmd_value(self, command_path):
        try:
    ...
    def _handle_reset(self, _request, response):
        if self._undervoltage_active():
            ...
            response.message = 'Undervoltage is still active. Fix the power supply before resetting safety.'
