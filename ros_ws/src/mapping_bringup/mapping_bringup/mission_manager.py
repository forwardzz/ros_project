...
class MissionManager(Node):
    ...
    def __init__(self):
        ...
        self.tsp_enabled = bool(self.declare_parameter("use_tsp", True).value)
        ...
        self.create_subscription(
            LaserScan, TOPIC_SCAN, self._scan_cb, qos_profile_sensor_data
        )
        self.create_subscription(
        ...
        self.mission_status_typed_pub = self.create_publisher(
            MissionStatus, TOPIC_MISSION_STATUS_TYPED, latched_qos()
        )
        ...
        self.create_service(SetBool, SERVICE_SET_TSP_MODE, self._handle_set_tsp_mode)
        self.create_service(
            Trigger,
            SERVICE_CLEAR_INSPECTION_REGIONS,
    ...
    def _recompute_rviz_plan(self):
        ...
        self.rviz_preview_path = []
        self.rviz_solving_method = "planning"
        if not self.rviz_points:
        ...
        if self.map_msg is not None and self.have_map_pose:
            self._queue_rviz_plan(
                points=self.rviz_points,
                source="rviz",
                optimize_order=self.tsp_enabled,
            )
        self._publish_rviz_plan_visuals()
    ...
    def _start_queued_rviz_plan(self):
        ...
        planner = plan_mission_order if request["optimize_order"] else preview_current_order
    ...
    def _poll_rviz_plan(self):
        ...
        if request is not None and request["generation"] == self.rviz_plan_generation:
            if error is not None:
                mode = "TSP" if request["optimize_order"] else "ordered"
                self.get_logger().error(f"Waypoint {mode} planning failed: {error}")
    ...
    def _apply_completed_rviz_plan(self, request, plan):
        ...
        if plan is None:
            ...
            self.rviz_preview_path = []
            self.rviz_solving_method = ""
            route_name = "TSP route" if request["optimize_order"] else "route in the selected order"
            message = f"Waypoint mission has no complete collision-free {route_name}"
            self.get_logger().warn(message)
        else:
            self.rviz_ordered_points = ordered_points
            self.rviz_preview_path = list(plan.preview_path)
            self.rviz_solving_method = (
                plan.solving_method if request["optimize_order"] else "selected_order"
            )
            names = " -> ".join(point.point_name for point in ordered_points)
            self.get_logger().info(
                f"Waypoint mission preview ready with {self.rviz_solving_method}: {names}"
            )
        self._publish_rviz_plan_visuals()
    ...
    def _finish_pending_rviz_start(self, request, plan, ordered_points):
        ...
        if plan is None:
            self._publish_zero_cmd()
            self._warn_safety(
                "Waypoint mission rejected: no complete collision-free route is "
        ...
        if not validation.valid:
            self._publish_zero_cmd()
            self._warn_safety(f"Waypoint mission rejected: {validation.message}")
            return
        ...
        home_pose = request["start_request"].get("home_pose")
        ...
        if not self._start_sequential_mission(
            ...
            self._publish_zero_cmd()
            self._warn_safety("Failed to start the planned waypoint mission")
            return
        ...
        message = (
            f"Mission started with {len(ordered_points)} points from {source}, "
            f"pause={pause_sec:.1f}s, ordering={self.rviz_solving_method}, "
            f"return_to_start={str(return_to_start).lower()}: {names}"
        )
        self.get_logger().info(message)
        self._publish_mission_status(message, state="RUNNING")
    ...
    def _recompute_region_preview(self):
        ...
        self.region_transition_paths = []
        self.region_return_path = []
        self.region_solving_method = "planning" if self.tsp_enabled else "selected_order"
        self.region_preview_path = []
        if (
            ...
            if self.tsp_enabled:
                ...
                prepared = self._prepare_sequential_region_plan()
                if prepared is not None:
                    self._apply_sequential_region_preview(prepared)
        self._publish_rviz_plan_visuals()
    ...
    def _select_region_staging(self, region, route):
        ...
        offset = self.region_margin + self.region_staging_distance
        for reverse in (False, True):
            ordered = MissionManager._clone_region_route(self, route, reverse=reverse)
            first, second = ordered[0], ordered[1]
            dx, dy = second.x - first.x, second.y - first.y
            length = math.hypot(dx, dy)
            if length < 1e-6:
            ...
            if not grid.is_valid(gx, gy):
                continue
            heading = math.atan2(first.y - sy, first.x - sx)
            staging = make_inspection_point(
                f"{region.name}_STAGING", sx, sy, heading
            )
            distance = math.hypot(
                sx - self.current_map_pose["x"], sy - self.current_map_pose["y"]
            )
            candidates.append((distance, ordered, staging))
        if not candidates:
            return None
        _, selected_route, staging = min(candidates, key=lambda item: item[0])
        return selected_route, staging

    def _prepare_sequential_region_plan(self):
        base_paths = generate_chassis_region_paths(
            self.inspection_regions, self.sweep_spacing, self.region_margin
        )
        if not base_paths or any(not path for path in base_paths):
        ...
        stagings = []
        transitions = []
        cursor = (self.current_map_pose["x"], self.current_map_pose["y"])
        for region, route in zip(self.inspection_regions, base_paths):
            ...
            if prepared is None:
                ...
                preview = preview_current_order(
                    self.map_msg, cursor, [staging], smooth=True
                )
                transition = list(preview.preview_path) if preview is not None else []
            paths.append(selected_route)
            stagings.append(staging)
            transitions.append(transition)
            if selected_route:
                cursor = (selected_route[-1].x, selected_route[-1].y)
        return {
    ...
    def _apply_sequential_region_preview(self, prepared):
        self.region_ordered_indices = list(range(len(prepared["regions"])))
        self.region_ordered_paths = list(prepared["paths"])
        self.region_ordered_staging_points = list(prepared["stagings"])
        self.region_transition_paths = list(prepared["transitions"])
        self.region_return_path = []
        self.region_solving_method = "selected_order"
        self.region_preview_path = self._compose_region_preview_path(
            self.region_ordered_paths, self.region_transition_paths, []
        )
    ...
    def _queue_region_plan(self, start_request=None):
        ...
        return_to_start = bool(start_request and start_request.get("return_to_start", False))
        home_pose = start_request.get("home_pose") if start_request else None
        request = {
            "generation": generation,
            "map_msg": self.map_msg,
            "start_pose": (
            ...
            "end_pose": tuple(home_pose) if return_to_start and home_pose else None,
            "regions": list(self.inspection_regions),
    ...
    def _publish_mission_status(self, message, safety=False, state=None):
        msg = String()
        ...
        if state is None:
            if self.mission_active:
                state = "WARNING" if safety else "RUNNING"
            elif safety:
                state = "FAILED"
            elif "abort" in message.lower() or "cancel" in message.lower():
                state = "CANCELED"
                ...
                state = "IDLE"
        typed.state = state
        typed.mode = self.mission_source or ("region" if self.region_mode else "waypoints")
        typed.message = message
        planning_active = (
            self.pending_rviz_start is not None or self.pending_region_start is not None
        )
        typed.active = self.mission_active or planning_active
        typed.safety_warning = bool(safety)
        if self.pending_region_start is not None:
            typed.mode = "region"
            typed.current_index = 0
            typed.total_count = len(self.pending_region_start.get("regions", []))
        elif self.pending_rviz_start is not None:
            typed.mode = "waypoints"
            typed.current_index = 0
            typed.total_count = len(self.pending_rviz_start.get("points", []))
        elif self.mission_source == "region":
            typed.current_index = max(0, int(self.region_path_index))
            typed.total_count = len(self.region_paths) or len(self.inspection_regions)
        else:
            typed.current_index = max(0, int(self.mission_index))
            typed.total_count = len(self.mission_points)
    ...
    def _publish_rviz_plan_visuals(self):
        ...
        for index, point in enumerate(self.rviz_ordered_points, start=1):
            ...
            text.color.a = 1.0
            text.text = (
                "…" if self.rviz_solving_method == "planning" else str(index)
            )
        ...
        for original_index, region in enumerate(self.inspection_regions):
            self._append_region_markers(
                ...
                if self.region_solving_method != "planning"
                else None,
            )
    ...
    def _handle_set_tsp_mode(self, request, response):
        enabled = bool(request.data)
        if self.mission_active or self.direct_nav_active:
            response.success = False
            response.message = "Cannot change TSP mode while a navigation task is active"
            return response
        self._cancel_pending_rviz_start(
            "Pending waypoint planning cancelled because TSP mode changed",
            publish=False,
        )
        self._cancel_pending_region_start(
            "Pending region planning cancelled because TSP mode changed",
            publish=False,
        )
        self.tsp_enabled = enabled
        self._recompute_rviz_plan()
        self._recompute_region_preview()
        response.success = True
        response.message = (
            "TSP optimization enabled; previews and new missions will be optimized"
            if enabled
            else "TSP optimization disabled; selected point and region order will be preserved"
        )
        self.get_logger().info(response.message)
    ...
    def _handle_region_sensor_failure(self, reason):
        ...
        if failure_limit_reached(self.tf_consecutive_failures):
    ...
    def _handle_abort_mission(self, _request, response):
        ...
        if self.region_approach_goal_handle is not None:
            try:
                self.region_approach_goal_handle.cancel_goal_async()
            except Exception as exc:
                errors.append(f"region approach: {exc}")
            else:
                aborted.append("region approach")
            self.region_approach_goal_handle = None
        ...
        if aborted:
            ...
            response.success = True
            response.message = "No active autonomous mission; stop command sent"
        self._publish_mission_status(response.message, state="CANCELED" if aborted else "IDLE")
        return response
    ...
    def _handle_start_navigation(self, request, response):
        ...
        use_tsp = bool(getattr(request, "use_tsp", self.tsp_enabled))
        if use_tsp != self.tsp_enabled:
            self.tsp_enabled = use_tsp
            self._invalidate_rviz_plans()
            self._invalidate_region_plans()

        if self.region_mode and not request.waypoints and not self.inspection_regions:
            response.success = False
            response.message = "Region Mode is enabled but no inspection regions are defined"
            return response

        region_points = []
        if should_run_regions(self.region_mode, request.waypoints, self.inspection_regions):
            self._recompute_region_preview()
        ...
        if return_to_start:
            home_valid, home_message = self._validate_mission_home(home_pose)
            if not home_valid:
                response.success = False
        ...
        if source == "region":
            ...
            if use_tsp:
                ...
                response.message = (
                    ...
                    "the robot will remain stopped until the current plan is ready; "
                    f"return_to_start={str(return_to_start).lower()}"
                )
                self.get_logger().info(response.message)
                self._publish_mission_status(response.message, state="PLANNING")
                return response
            ...
            response.success = True
            response.message = (
                f"Region mission started in selected order with {len(prepared['regions'])} region(s), "
                f"return_to_start={str(return_to_start).lower()}"
            )
            self.get_logger().info(response.message)
            self._publish_mission_status(response.message, state="RUNNING")
            return response
        ...
        response.success = True
        ordering = "TSP" if use_tsp else "selected-order"
        response.message = (
            f"{ordering} planning accepted for {len(points)} point(s) from {source}; "
            "the robot will remain stopped until the current plan is ready; "
            f"return_to_start={str(return_to_start).lower()}"
        )
        self.get_logger().info(response.message)
    ...
    def _clamp_waypoint_pause(self, value):
        return clamp_waypoint_pause(value, self.DEFAULT_WAYPOINT_PAUSE_SEC)

    def _validate_mission_home(self, home_pose):
        values = tuple(home_pose)
        if not all(math.isfinite(value) for value in values):
            return False, "Return-to-start is unavailable: task start pose is invalid"
        home_x, home_y, _home_yaw = values
        grid_map = GridMap.from_occupancy_grid(self.map_msg)
        gx, gy = grid_map.world_to_grid(home_x, home_y)
        if not grid_map.in_bounds(gx, gy):
            return (
                False,
                "Return-to-start is unavailable: task start pose "
                f"({home_x:.2f}, {home_y:.2f}) is outside map bounds",
            )
        if not grid_map.is_valid(gx, gy):
            return (
                False,
                "Return-to-start is unavailable: task start pose "
                f"({home_x:.2f}, {home_y:.2f}) is occupied or unknown",
            )
        return True, "Task start pose is valid"
    ...
    def _start_region_path_mission(
        ...
        if home_pose is not None:
            self.mission_home_x, self.mission_home_y, self.mission_home_yaw = home_pose
        self.mission_run_id += 1
    ...
    def _send_region_approach_goal(self):
        ...
        try:
            ...
            self._skip_current_region(f"Nav2 staging request failed: {exc}")
            return True
    ...
    def _region_approach_response_cb(self, future, run_id):
        ...
        if not handle.accepted:
            self._skip_current_region("Nav2 staging goal was rejected")
            return
    ...
    def _region_approach_result_cb(self, future, run_id, expected_handle):
        ...
        try:
            ...
            self._skip_current_region(f"Nav2 staging result failed: {exc}")
            return
        if result.error_code != NavigateToPose.Result.NONE:
            self._skip_current_region(
                f"Nav2 staging failed with code {result.error_code}: {result.error_msg}"
            )
            return
    ...
    def _send_current_mission_goal(self):
        ...
        self._publish_mission_status(
            f"Navigating to waypoint {waypoint_number}/{total}: {point.point_name}",
            state="RUNNING",
        )
        return True
    ...
    def _is_current_mission_goal(self, run_id, waypoint_index):
        return mission_callback_is_current(
            self.mission_active,
            run_id,
            self.mission_run_id,
            waypoint_index,
    ...
    def _publish_mission_snapshot(self):
        ...
        if self.mission_active and self.mission_source == "region":
            self._publish_mission_status(
                f"Region {self.region_path_index + 1}/{len(self.region_paths)}: "
                f"{self.region_phase or 'approach'}",
                state="RUNNING",
            )
        stamp = self.get_clock().now().to_msg()
        marker_array = MarkerArray()
    ...
    def _start_return_to_start(self):
        ...
        goal.behavior_tree = ""
        message = (
            "Inspection completed successfully; returning to task start pose "
    ...
    def _finalize_mission_success(self, message):
        ...
        self._publish_zero_cmd()
        self.get_logger().info(message)
        self._publish_mission_status(message, state="COMPLETED")
        self._clear_mission_state()
    ...
    def _finish_mission_failed(self, message):
        ...
        self._publish_mission_status(message, safety=True, state="FAILED")
        self._clear_mission_state()
