...
class SafetyMonitor(Node):
    def __init__(self):
        ...
        self.stop_pub = self.create_publisher(Bool, "/safety_stop", latched_qos)
        ...
        self.create_service(SetBool, "/emergency_stop", self._handle_emergency_stop)
        ...
        self.create_subscription(MissionStatus, "/mission_status_typed", self._mission_cb, 10)
    ...
    def _mission_cb(self, msg):
        ...
        if not self.mission_active and not self.fault_active:
            self.abort_requested = False
    ...
    def _handle_emergency_stop(self, request, response):
        """Latch a software E-stop; clearing requires an explicit safe reset."""
        if not bool(request.data):
            response.success = False
            response.message = (
                "Emergency stop is latched. Inspect the vehicle, then call "
                "/reset_safety_monitor to re-enable motion."
            )
            return response
        self.emergency_stop_latched = True
        self._trigger_fault(
            "EMERGENCY_STOP",
            "Software emergency stop requested. Mission aborted and motion inhibited.",
        )
        response.success = True
    ...
    def destroy_node(self):
        # A supervisor that is deliberately stopped must leave the velocity
        # gate in a fail-safe state immediately, rather than waiting for its
        # status watchdog to expire.
        if rclpy.ok():
            self.fault_active = True
            self.level = "FAULT"
            self.code = "SAFETY_MONITOR_EXIT"
            self.message = "Safety monitor exited; motion inhibited"
            self._publish_status()
        super().destroy_node()
